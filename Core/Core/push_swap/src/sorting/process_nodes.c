...
void push_range_to_stack_b(t_node **stack_a, t_node **stack_b, int lower_bound, int upper_bound)
{
    int pushed_count = 0;
    int total_pushed = 0;
    ...
    while (*stack_a && pushed_count < total_pushed)
    {
       t_node *current = *stack_a;
        ...
        else if (current->next->data >= lower_bound && current->next->data <= upper_bound)
        {
            sa(stack_a);
            pb(stack_a, stack_b);
            pushed_count++;
        }
        else if (last_node->data >= lower_bound && last_node->data <= upper_bound)
        {
            rra(stack_a); 
            pb(stack_a, stack_b);
            pushed_count++;
        }
        else if (second_last_node->data >= lower_bound && second_last_node->data <= upper_bound)
        {
            rra(stack_a); 
            rra(stack_a);
            pb(stack_a, stack_b); 
        ...
        {
            if (pushed_count < total_pushed)
                ra(stack_a);
        }
        if (pushed_count >= (upper_bound - lower_bound + 1))
            break;
    }
}
...
void process_nodes_with_fixed_ranges(t_node **stack_a, t_node **stack_b, int total_elements) {
    ...
    while (i < 5)
    {
        ...
        push_range_to_stack_b(stack_a, stack_b, lower_bound, upper_bound);

        int current_median = (lower_bound + upper_bound) / 2;
        rotate_stack_b_based_on_median(stack_b, current_median);
